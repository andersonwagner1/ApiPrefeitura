...
public class SofPlanDao{
	...
	public void montagemDosDados() throws SQLException{
		...
		while(resultSet.next()){
			...
			//}
				...
				String inscricaoImovel = ESTRUTURA;
				String valor = resultSet.getString("INCRICAO_IMOVEL");
				
				String solicitante = resultSet.getString("IDENTIFICACAO_INSCRICAO");
				if(solicitante == null){
					solicitante = "";
				}
				
				inscricaoImovel = inscricaoImovel.replace("$campo", solicitante);		
				inscricaoImovel = inscricaoImovel.replace("$valor", valor == null? "" : valor);
			...
			resultadoFormulario = inscricaoImovel + resultadoFormulario;
			AdmCargaFormulario formulario = new AdmCargaFormulario();
	...
	private Map<String, String[]> localizarNomeCampos(String nomeTabela) throws SQLException{
		...
		Map<String, String[]> listaCamposAux = new HashMap<String, String[]>();
		while(resultSet.next()){
			listaCamposAux.put(resultSet.getString("column_name"), retonarNomeCampos(resultSet.getString("column_name")));
		}
		resultSet.close();
	...
	private String[] retonarNomeCampos(String id) throws SQLException{
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT COLUNAS.NM_ATT_MATERIALIZADO, COLUNAS.NOMEDEEXIBICAO, COLUNAS.ATRIBUTO_ID, COLUNAS.NOMESECAO");
		...
		String[] nomeExibido = new String[3];
		if(resultSet.next()){
			nomeExibido[0] = resultSet.getString("NOMEDEEXIBICAO");
			nomeExibido[1] = resultSet.getString("NOMESECAO");
			nomeExibido[2] = resultSet.getString("ATRIBUTO_ID");
			resultSet.close();
		
			return nomeExibido;
		}
		resultSet.close();
		nomeExibido [0] = "Falha me localizar o nome do campo " + id + " ";
		return nomeExibido;		
	}
	...
	private String localizarValoresCampos(Long idDocumentoBase, String tabela, Map<String, String[]> campos) throws SQLException{
		...
		//boolean chekcAtivo = false;
		//String nomeCampoAux = ""; // usado para checkbox
		//String valorCampoAux = ""; // usado para checkbox
		if(resultSet.next()){
			if(campos == null){
				...
				for (String key : campos.keySet()) {
					...
					//if(nomeCampo[2].equals("41")){ // Quando o campo for um check box alterar a situação
					//	nomeCampoAux = nomeCampo[1]; // pega o nome do campo da sessão
					//	chekcAtivo = true; // verfica se o checkt esta ativo
					//	if(valorCampo.equals("1")){
					//		valorCampoAux += nomeCampo[0] + "-"; // marca os campos selecionados
					//	}
					//}else{
						//Significa que o registro anterior era um checkbox e foi finalizado, então o sistema deverá 
						// pegar o valor armazedo e adicionar na lista de rgisro
						//if(chekcAtivo){
						//	chekcAtivo = false;
						//	String linha = ESTRUTURA;
						//	linha = linha.replace("$campo", nomeCampoAux);
						//	linha = linha.replace("$valor", valorCampoAux == null? "" : valorCampoAux);
						//	formatacao.append(linha);
							
						//	nomeCampoAux = ""; //limpa os campos do checkbox
						//	valorCampoAux = "";
						//}
						
						
						String linha = ESTRUTURA;
						linha = linha.replace("$campo", nomeCampo[0]);
		...
		//Quando for fim de campo tambem zerar
		/*if(chekcAtivo){
			chekcAtivo = false;
			String linha = ESTRUTURA;
			linha = linha.replace("$campo", nomeCampoAux);
...
//		System.out.println(formatacao);
	...
	private ResultSet localizarTodosProcessos(){
		...
		sql.append(" WHERE E.ID_DOCUMENTO_BASE not in(SELECT F.ID_DOC_BASE FROM ADM_CARGA_FORMULARIO F WHERE OBSERVACAO IS NOT NULL) ");
		...
		sql.append(" OR E.DATA >= '13-04-2023'"); //04/04/2023
